...
export default function MapClient() {
  ...
  function buildReportMd(): string {
    ...
    md += "## Filter State\n\n";
    md += `- **Directory**: ${filterState.dir}\n`;
    md += `- **Extension**: ${filterState.ext}\n`;
    md += `- **Hide Isolated**: ${filterState.hideIsolated}\n`;
    md += `- **Hub Threshold**: ≥ ${filterState.hubThreshold}\n`;
    md += `- **Safe Mode**: ${filterState.safeMode ? "ON" : "OFF"} (threshold: ${filterState.safeThreshold})\n`;
    md += "\n";

    md += "## Focus State\n\n";
    md += `- **Focus ID**: ${focusState.focusId}\n`;
    md += `- **Focus Only**: ${focusState.focusOnly}\n`;
    md += "\n";
  ...
  async function copyReport() {
    try {
      ...
      alert("Failed to copy report: " + String(err));
    }
  }
  ...
  function downloadReport() {
    try {
      ...
      a.href = url;
      a.download = `neural-map-report-${new Date().toISOString().slice(0, 19).replace(/:/g, "-")}.md`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch (err) {
      alert("Failed to download report: " + String(err));
    }
  }
